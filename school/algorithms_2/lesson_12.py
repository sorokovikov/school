...
class SimpleGraph:
    ...
    def WeakVertices(self) -> list[Vertex]:
        ...
        def is_in_triangle(vertex_index: int) -> bool:

            self.vertex[vertex_index].visit()
            neighbours = self.__get_adjacent_vertices(vertex_index)

            for neighbour in neighbours:
                for adjacent_to_neighbour in self.__get_adjacent_vertices(neighbour):
                    if adjacent_to_neighbour != vertex_index and adjacent_to_neighbour in neighbours:
                        self.vertex[neighbour].visit()
                        self.vertex[adjacent_to_neighbour].visit()
                        return True
            return False
        ...
        for i, vertex in enumerate(self.vertex):
            if vertex is not None and not vertex.is_visited() and not is_in_triangle(i):
